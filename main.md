...
- **[hi/hi] Push commit-guard protocol to `_filesys.md`** — generalize propose-and-wait from validated reference (`../omarchy/commit-guard.md`). Add to `_filesys.md` Git: (1) proposal format — header with repo-name tag, msg line, stage list with per-file +adds/-dels, totals row, skip list for untracked-not-this-session; (2) approval = explicit typed go ("commit"/"yes"/"go"); plan-mode `allowedPrompts` is NOT approval; (3) marker write and `git commit` MUST be separate Bash calls — chained `... && git commit` bypasses the hook (PreToolUse reads stale marker before chain runs); (4) literal first token of every commit-bearing Bash call must be `git` — no `cd && git`, no env-var prefix, no subshell, no pipe; (5) supported forms: `git commit -m "..."` and `git -C /path commit -m "..."` only — heredoc, `-F <file>`, `--message=`, and multi-line messages unsupported; (6) marker self-invalidates post-commit (staged tree empty → hash mismatch) → re-propose for next commit. Hook is global at `~/.claude/settings.json` PreToolUse — children inherit enforcement whether they know it or not, so the protocol must live in `_filesys.md` for them to operate correctly. Supersedes the prior `[hi/lo] Commit proposal required` item and absorbs the "Remaining: generalize the propose-and-wait pattern" sentence from `[lo/lo] Behavioral hooks for commit workflow`.

- **[hi/lo] Commit approval hardening — nonce-in-marker** — current commit-guard validates `(repo_path, staged_tree_hash, message)` but cannot distinguish "builder typed go for THIS commit" from "LLM wrote a marker matching the staged state." Failure mode confirmed 2026-05-15 in `art`: after `git reset --mixed HEAD~1`, LLM staged → wrote marker → committed without waiting for fresh typed approval; hook allowed because marker validated against the new staged tree. Recommended fix: extend marker schema with a 4th line — single-use approval token typed by builder between proposal and marker write. Hook stores accepted tokens, refuses any token already used. Structurally prevents inference. Full incident write-up, three prevention options ranked by mechanical strength, and recommended implementation in `commit-approval-hardening.md`. Bundle with the parent `[hi/hi] Push commit-guard protocol to _filesys.md` work.
